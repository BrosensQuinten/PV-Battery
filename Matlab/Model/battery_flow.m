function [pf,injectie,consumptie, battery_charge] = battery_flow(Efficiency,irr,load, surface_area, battery_capacity)
    ...
    while j == 0
        ...
        if pf(index,1) > 0
            if pf(index, 1) + battery_charge(index, 1) < battery_capacity
                battery_charge(index+1,1) = battery_charge(index,1) + pf(index,1);
                pf(index,1) = 0;
            else
                pf(index,1) = pf(index,1) + battery_charge(index,1) - battery_capacity;
            ...
            if pf(index, 1) + battery_charge(index, 1) > 0
                ...
                pf(index,1) = pf(index,1) + battery_charge(index,1);
